...
public class DbToolsUtils {
    ...
    private static String extractDatabaseTypeFromUrl(DbTable currentTable) {
        RawConnectionConfig config = currentTable.getDataSource().getConnectionConfig();
    ...
    public static IntellijColumnInfo convertColumnToIntellijColumnInfo(DasColumn column, String databaseType) {
        ...
        // 检查是否是 DasDataBuiltinType 类型
        if (dasType instanceof DasDataBuiltinType) {
            // 针对 DasDataBuiltinType 特殊处理
            typeName = "UNKNOWN";
        } else {
            // 根据数据库类型获取类型信息
            String typeStr = dasType.toString();
            if ("Oracle".equals(databaseType)) {
                // Oracle 数据库特殊处理
                typeName = typeStr.split("\\(")[0].trim().toUpperCase();

            } else {
                // 其他数据库
                typeName = typeStr.split("\\s+")[0].trim().toUpperCase();
            }
        }

        columnInfo.setDataType(convertTypeNameToJdbcType(typeName, databaseType));
        ...
        // 从 DasType 的字符串表示中提取大小和精度信息
        String typeStr = dasType.toString();
        Matcher matcher = Pattern.compile(".*\\((\\d+)(?:,\\s*(\\d+))?\\).*").matcher(typeStr);
        if (matcher.matches()) {
            columnInfo.setSize(Integer.parseInt(matcher.group(1)));
            if (matcher.group(2) != null) {
                columnInfo.setDecimalDigits(Integer.parseInt(matcher.group(2)));
            }
        } else {
